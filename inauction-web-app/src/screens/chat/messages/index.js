...
class MessagesList extends Component {
    ...
    Communication = () => {
        ...
        return <div className="gx-chat-main">
            {/* selected header user, chat header */}
            <div className="gx-chat-main-header">
                <span className="gx-d-block gx-d-lg-none gx-chat-btn">
                    <i className="gx-icon-btn icon icon-chat" onClick={this.onToggleDrawer.bind(this)} />
                </span>
                <div className="gx-chat-main-header-info">
                    <div className="gx-chat-avatar gx-mr-2">
                        <div className="gx-status-pos">
                            <Avatar src={selectedUser.thumb} className="gx-rounded-circle gx-size-60" alt="" />
            ...
            {/* converstatio with selected user */}
            <CustomScrollbars className="gx-chat-list-scroll">
                <Conversation conversationData={conversationData} selectedUser={selectedUser} />
            </CustomScrollbars>
            ...
            <div className="gx-chat-main-footer">
                <div className="gx-flex-row gx-align-items-center" style={{ maxHeight: 51 }}>
                    ...
                    <i className="gx-icon-btn icon icon-sent" onClick={this.submitMessage.bind(this)} />
                </div>
            </div>
        ...
        return <div className="gx-chat-sidenav-main">
            <div className="gx-bg-grey-light gx-chat-sidenav-header">
                ...
                <div className="gx-chat-user-hd gx-mb-0">
                    <i className="gx-icon-btn icon icon-arrow-left" onClick={() => { this.setState({ userState: 1 }); }} />
                </div>
                ...
                <div className="gx-chat-user-hd">
                    ...
                    <div className="gx-module-user-info gx-flex-column gx-justify-content-center">
                        <div className="gx-module-title">
                            <h5 className="gx-mb-0">Eglon Metalia</h5>
                        </div>
                        <div className="gx-module-user-detail">
                            <span className="gx-text-grey gx-link">metalia.eglon@live.com</span>
                        </div>
                    </div>
                </div>
                ...
                {this.state.selectedUser === null ?
                    <div className="gx-comment-box">
                        ...
                        <Button className="gx-d-block gx-d-lg-none" type="primary"
                            onClick={this.onToggleDrawer.bind(this)}>{<IntlMessages id="chat.selectContactChat" />}
                        </Button>
                    </div>
                    : this.Communication()}
            </div>)
    ...
    getContactList() {
        //todo: call api
        this.setState({ contactList: users.filter((user) => !user.recent) })
    }
    ...
    getChatUsers() {
        //todo : call api
        this.setState({ chatUsers: users.filter((user) => user.recent) })
    }

    // filter list of contact
    ...
    // on enter click call submit function
    _handleKeyPress = (e) => {
    ...
    submitMessage() {
        if (this.state.message !== '') {
            ...
            const messageObject = this.state.conversation.conversationData.concat({
                'type': 'sent',
            ...
            this.setState({
                conversation: {
                    ...this.state.conversation, conversationData: messageObject
                },
                message: '',
                conversationList: this.state.conversationList.map(conversationData => {
                    if (conversationData.id === this.state.conversation.id) {
                        return { ...this.state.conversation, conversationData: messageObject };
                    } else {
    ...
    render() {
        ...
        return (
            <Auxiliary>
                <Helmet>
                    <title>iNauction Tool | {customer.subrole.description} | All Messages</title>
                </Helmet>
                <Breadcrumbs description={"Chat Module - Messages"} name={"Messages"} />
                <div className="ant-card gx-card  ant-card-bordered">
                    <div className=" gx-chat-module">
                        <div className="gx-chat-module-box">
                            <div className="gx-d-block gx-d-lg-none">
                                <Drawer
                                    ...
                                    {userState === 1 ? this.ChatUsers() : this.UsersInfo()}
                                </Drawer>
                            </div>
